...
start game start_state ws = 
    let (wins, losses) = ws in
    do
        putStrLn("Stats: "++ show wins++ " wins " ++show losses++" losses")
        putStrLn("Would you like to exit?")
        line <- getLine
        if line == "yes"
            then
                do
...
playGame game (Action answer) (EndOfGame val start_state) ws =
    do
        ...
        if(val == 1) 
            then 
                do displayAnswer "Congratulations :)" answer
            else 
                do displayAnswer "Nice try :)" answer
...
main = start wordle wordle_start (0, 0)
